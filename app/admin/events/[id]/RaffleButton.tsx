...
export default function RaffleAnimation({ participants, onRaffleComplete }: RaffleAnimationProps) {
  ...
  const totalPlaces = 3 // Можно изменить под количество призов
  ...
  const startRaffle = async () => {
    ...
    try {
      ...
      // Получаем список победителей из ответа
      const winnerIds = data.winners || []
      
      // Запускаем анимацию для каждого места
  ...
  const animateForPlace = async (winnerIds: string[], place: number) => {
    ...
    await new Promise<void>((resolve) => {
      ...
      const runFinal = () => {
        if (steps < maxSteps) {
          // Последние шаги — ближе к победителю
